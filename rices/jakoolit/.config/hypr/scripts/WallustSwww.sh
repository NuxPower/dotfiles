...
dashboard_wallpaper="$HOME/.cache/rofi/dashboard-wallpaper.png"
...
# Rofi cannot position a scaled background image. Prepare a center-cropped
# copy with the same aspect ratio as the dashboard theme's left pane.
prepare_dashboard_wallpaper() {
  command -v magick >/dev/null 2>&1 || return 0

  local monitor_width=1920 monitor_height=1080 monitor_data target_width target_height tmp
  if command -v jq >/dev/null 2>&1 && monitor_data="$(hyprctl monitors -j 2>/dev/null)"; then
    monitor_width="$(jq -r '.[] | select(.focused) | .width // empty' <<<"$monitor_data")"
    monitor_height="$(jq -r '.[] | select(.focused) | .height // empty' <<<"$monitor_data")"
  fi

  [[ $monitor_width =~ ^[0-9]+$ ]] || monitor_width=1920
  [[ $monitor_height =~ ^[0-9]+$ ]] || monitor_height=1080

  # Dashboard window: 62% x 52% of the monitor; left pane: 33% of monitor width.
  target_width=$((monitor_width * 33 / 100))
  target_height=$((monitor_height * 52 / 100))
  mkdir -p "$(dirname "$dashboard_wallpaper")"
  tmp="$dashboard_wallpaper.tmp.png"

  if magick "${wallpaper_path}[0]" -auto-orient \
      -resize "${target_width}x${target_height}^" \
      -gravity center -extent "${target_width}x${target_height}" \
      "$tmp"; then
    mv "$tmp" "$dashboard_wallpaper"
  fi
}
...
prepare_dashboard_wallpaper || true
